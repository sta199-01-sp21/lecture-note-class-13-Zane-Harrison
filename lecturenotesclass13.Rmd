...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-Zane-Harrison/manhattan.csv")
```
...
With repeaded sampling, we can be 95% confident that the interval from $2162 to 
$2875 captures the true median rent price of Manhattan 1-bedroom apartments.
...
- Sample: Untreated mice that died
- Sample size: 522
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(live_prop, level = .90)
ci_mice_lived
```
...
After repeated sampling, we can be 90% confident that the interval from 0.552 to
0.651 captures the true proportion of mice that lived while on the treatment, 
